...
public class axialPanel : MonoBehaviour {
		...
		for(int i = 0; i<3;i++){
			for(int j = 0; j<10;j++){
				...
				if(i==1){
					...
					widthText[j].text = dS.mode[i][j].ToString("f1");
				...
				if(i==2){
					...
					heightText[j].text = dS.mode[i][j].ToString("f1");
		...
		for(int i = 0; i<3;i++){
			for(int j = 0; j<10;j++){
				if(i==0){
					...
					if(dS.mode[i][j] > sFreqMin && dS.mode[i][j] < sFreqMax){
							lengthText[j].text = dS.mode[i][j].ToString("f1");
